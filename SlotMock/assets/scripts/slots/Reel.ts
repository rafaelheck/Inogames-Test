...
export default class Reel extends cc.Component {
  ...
  changeCallback(element: cc.Node = null): void {
    ...
    if (el.position.y * dirModifier > 288) {
      ...
      if (pop !== null && pop["texture"] >= 0) {
        if(pop["winnerTile"]){ //If the tile is from the winner line, add the glow effect on it
          el.getComponent('Tile').startGlow();
        }
        el.getComponent('Tile').setTile(pop["texture"]);
      } else {
